...
const validateForm = () => {
  errors.value = []
  if (!form.value.title?.trim()) errors.value.push(t('projects.validation.titleRequired'))
  if (!form.value.assignedID) errors.value.push(t('projects.validation.teamMemberRequired'))
...
const handleUpdateTask = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: t('projects.toast.success'), detail: t('projects.toast.taskUpdated'), life: 3000 })
    onClose()
    if (props.onUpdated) props.onUpdated()
    emit('taskUpdated')
  } catch (error) {
    console.error('Error updating task:', error)
    toast.add({ severity: 'error', summary: t('projects.toast.error'), detail: t('projects.toast.couldNotUpdateTask'), life: 3000 })
    errors.value = [t('projects.validation.updateTaskError')]
  } finally {
    loading.value = false
  }
}
  ...
  <div v-if="visible" class="modal-overlay" @click.self="onClose">
    <div class="modal-box">
      <div class="modal-header">
        <h2 class="modal-title">{{ $t('projects.editTask') }}</h2>
      ...
      <div class="modal-footer">
        <button type="button" class="btn-edit" @click="handleUpdateTask" :disabled="loading">
          ...
          <span v-else>{{ $t('projects.edit') }}</span>
        </button>
      </div>
    </div>
  </div>
