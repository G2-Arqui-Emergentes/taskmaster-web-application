...
import { createProject as createProjectApi } from "@/services/project.service.js";
import { useToast } from "primevue/usetoast";

const props = defineProps({ visible: { type: Boolean, default: false }, onCreated: Function });
...
const createProject = async () => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: t('projects.toast.success'), detail: t('projects.toast.projectCreated'), life: 3000 });
    resetForm();
    emit('update:visible', false);
    if (props.onCreated) props.onCreated();
  } catch (error) {
    console.error('Error creating project:', error);
    toast.add({ severity: 'error', summary: t('projects.toast.error'), detail: t('projects.toast.couldNotCreateProject'), life: 3000 });
    errors.value = [t('projects.validation.createProjectError')];
  } finally {
    loading.value = false;
  }
};
  ...
  <div v-if="visible" class="modal-overlay" @click.self="onClose">
    <div class="modal-box">
      <div class="modal-header">
        <h2 class="modal-title">{{ $t('projects.createNewProject') }}</h2>
      ...
      <div class="modal-footer">
        <button type="button" class="btn-create" @click="checkForm" :disabled="loading">
          <i v-if="loading" class="pi pi-spin pi-spinner"></i>
          <span v-else>{{ $t('projects.createProject') }}</span>
        </button>
      </div>
    </div>
  </div>
